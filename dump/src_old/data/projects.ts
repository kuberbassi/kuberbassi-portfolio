...
export const initialProjects: Project[] = [
  {
    ...
    fullDescription: 'Zenith is a modern academic platform designed to streamline course material access, collaborative study groups, and notes management.',
    overview: 'Built with React and Node.js, Zenith provides an intuitive interface for students and educators to organize knowledge seamlessly.',
    problem: 'Existing study platforms are fragmented, ad-heavy, and lack real-time collaborative organization.',
    solution: 'Unified, high-speed single-page application with instant search, structured subject trees, and clean Markdown notes rendering.',
  ...
  },
  {
    ...
    fullDescription: 'IndiaOnRoaming connects travelers with flexible eSIM data packages across regions with transparent pricing.',
    overview: 'Integrated with payment gateways and automated eSIM profile delivery services for friction-free travel setup.',
  ...
  },
  {
    ...
    fullDescription: 'An open-source React component built with GSAP and WebGL CSS 3D transforms to display projects as an ancient codex.',
    overview: 'High-performance 3D page flip physics curve with responsive layout and touch/keyboard controls.',
    problem: 'Standard portfolio grids feel repetitive and lack distinct visual storytelling.',
    solution: 'Immersive 3D interactive book interface with realistic parchment textures, custom binding stitches, and smooth state transitions.',
  ...
  },
  {
    ...
    fullDescription: 'Automatically captures track metadata from YouTube Music playback and submits scrobbles to Last.fm APIs.',
    overview: 'Lightweight Chrome Extension + background script with offline queuing and duplicate filtration.',
    problem: 'YouTube Music lacks native background scrobbling on desktop browsers and mobile web.',
    solution: 'Seamless background DOM observer with real-time Last.fm API handshake and playback notifications.',
  ...
  },
  {
    ...
    fullDescription: 'Adhikar AI uses LLM agentic retrieval to make complex legal statutes, citizen rights, and government schemes understandable.',
  ...
  },
  {
    ...
    fullDescription: 'WebGL 3D canvas rendering planetary orbits, constellation mappings, and astronomical data.',
...
export async function fetchGitHubRepos(username: string = 'kuberbassi'): Promise<Project[]> {
  try {
    const res = await fetch(`https://api.github.com/users/${username}/repos?sort=updated&per_page=30`);
    if (!res.ok) throw new Error(`GitHub API HTTP ${res.status}`);
    const data = await res.json();
    if (!Array.isArray(data)) return initialProjects;
    ...
    const formatted: Project[] = data
      ...
      .map((repo: any, index: number) => {
        ...
        const formattedTitle = repo.name
          .split(/[-_]/)
          .map((w: string) => (['ai', 'ui', 'api', 'cli', '3d'].includes(w.toLowerCase()) ? w.toUpperCase() : w.charAt(0).toUpperCase() + w.slice(1)))
          .join(' ');

        return {
          title: formattedTitle,
          slug,
          desc: repo.description || 'Public GitHub repository synced automatically.',
          tech: [repo.language || 'Code', 'GitHub API'].filter(Boolean),
          language: repo.language || 'Code',
          stars: repo.stargazers_count || 0,
          link: repo.homepage || (repo.has_pages ? `https://${username}.github.io/${repo.name}` : undefined),
          github: repo.html_url,
          projectId: `${slug.substring(0, 4).toUpperCase()}-${String((index + 1) * 101).padStart(3, '0')}`,
          version: 'main',
          stat: repo.homepage ? 'LIVE' : 'SOURCE',
          featured: repo.stargazers_count > 3,
          category: repo.language === 'Python' ? 'ai' : 'fullstack',
          year: new Date(repo.updated_at).getFullYear().toString(),
        };
      });

    return formatted.length > 0 ? formatted : initialProjects;
  } catch (err) {
    console.warn('GitHub API sync failed, loading fallback catalog:', err);
