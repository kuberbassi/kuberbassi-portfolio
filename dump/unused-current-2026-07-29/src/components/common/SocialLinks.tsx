...
/* Brand icons as inline SVGs since lucide-react v1 removed them */
...
const iconMap: Record<string, React.FC<SocialIconProps>> = {
  Github:    IconGithub,
  Linkedin:  IconLinkedin,
  Instagram: IconInstagram,
  Youtube:   IconYoutube,
  Mail:      ({ size }) => <Mail size={size} strokeWidth={1.5} />,
};
...
export function SocialLinks({ links, className, size = 18, direction = 'row' }: SocialLinksProps) {
  return (
    <div
      className={cn(
        'flex gap-4',
    ...
    >
      {links.map((link) => {
        const Icon = iconMap[link.icon] ?? (({ size: s }: SocialIconProps) => <Mail size={s} strokeWidth={1.5} />);
        return (
          <a
            key={link.label}
            href={link.href}
