...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const computedRadius = Number.parseFloat(
        window.getComputedStyle(btn).borderTopLeftRadius
      );
      const w = rect.width;
      const h = rect.height;
      sizeRef.w = w;
      sizeRef.h = h;
      sizeRef.radius = Number.isFinite(computedRadius) ? computedRadius : propsRef.current.radius;
  ...
  const classes = `specular-button relative m-0 inline-flex cursor-pointer items-center justify-center border-none font-medium leading-none tracking-[0.01em] no-underline outline-none disabled:cursor-default disabled:opacity-55 [color:var(--sb-text-color)] [border-radius:var(--sb-radius)] [background:color-mix(in_srgb,var(--sb-tint)_calc(var(--sb-tint-opacity)*100%),transparent)] [backdrop-filter:blur(var(--sb-blur))] shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_8px_24px_rgba(0,0,0,0.25)] focus-visible:outline-2 focus-visible:outline-offset-[3px] ${SIZES[size] || SIZES.md}${className ? ` ${className}` : ''}`;
  ...
  const sharedStyle = {
    ...
    // Keep the CSS silhouette and shader SDF on the same radius even inside
    // higher-specificity contexts such as the navbar.
    borderRadius: `${radius}px`
  } as CSSProperties;

  const content = (
    <>
      <span ref={fxRef} aria-hidden="true" className="specular-button__fx pointer-events-none absolute -inset-5 z-[1] [&_canvas]:block [&_canvas]:h-full [&_canvas]:w-full" />
      <span className="specular-button__content relative z-[2]">{children}</span>
    </>
  );
